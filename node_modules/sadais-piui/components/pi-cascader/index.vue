      ...
      <view class="cascader-wrap pi-flex-sub pi-flex-column">
        ...
        <scroll-view class="scroll-view" :scroll-left="scrollLeft" scroll-x scroll-with-animation>
          ...
          >
            ...
            >
              <view class="pi-flex-sub column-item-inner">
                <slot name="item" :item="item">
                  <view class="pi-align-center column-item-inner">
                    <pi-checkbox
                    ...
                    />
                    <view class="pi-align-center pi-lh-36 pi-flex-sub column-item-inner" @click="handleClickItem(item)">
                      {{ item[displayField] || '' }}
                    </view>
                  </view>
                </slot>
        ...
        <!-- 已选区域 -->
        <view v-if="!isSingle" class="cascader-area">
        ...
        <view v-else class="cascader-area">
          <text>已选：</text>
          <scroll-view
            class="node-breadcrumb pi-solid-bottom-1"
            scroll-x
            style="flex: 1;padding-left: 0;overflow: hidden;"
          >
            <view
              v-for="(item, index) in currentPathItems"
              :key="index"
              class="breadcrumb-item"
              @click="handleClickItem(item)"
            >
              <pi-icon v-if="index > 0" name="right" custom-class="pi-mg-lr-6" />
              <view
                class="breadcrumb-item-title"
              >
                {{ item[displayField] }}
...
export default {
  ...
  data() {
    ...
    return {
      rootItem,
      currentItem: this.$pi.lang.cloneDeep(rootItem),
      innerItems: [],
      itemsMap: {}, // 已选数据id索引
      selectedIds: [], // 已选数据
      cascaderWidth: this.$pi.system.systemInfo.screenWidth, // 容器宽度
  ...
  computed: {
    ...
    currentPathItems() {
      ...
      if (this.isSingle) {
        // 往上找构建完整路径
        let p = this.itemsMap[this.selectedIds[0]]
        while (p) {
          pathItems.unshift(p)
          p = this.itemsMap[p.pid]
        }
  ...
  methods: {
    rebuildItems() {
       ...
       * 递归调用太多次，导致栈溢出
       */
      this.currentItem = this.$pi.lang.cloneDeep(this.rootItem)
      const innerItems = []
      // 初始化堆栈 为items的根节点
    ...
    async syncScrollLeft() {
      ...
      if (this.cascaderWidth === 0) {
        const srollWrapRect = await this.$pi.common.queryRect(this, '.cascader-wrap', false)
        console.log(this, srollWrapRect)
        if (srollWrapRect) {
